...
## M1.03 — Registration consumer — отменено решением о synchronous provisioning

**Зависит от:** M1.01, M1.02.

- [x] обязательный профиль вынесен из Kafka consumer в идемпотентный internal API;
- [x] Profile + settings + completion фиксируются одной локальной transaction;
- [x] повтор Identity использует тот же user_id и durable completion.

**Готово:** повторный provisioning не создаёт дубликаты; Kafka event не является
условием существования профиля.

## M1.04 — Profile consumer process — отложено

Consumer может появиться только для независимых проекций; default profile через
него не создаётся.

- [ ] определить независимый event-driven use case;
- [ ] отдельный entrypoint/container role и operational contract;

**Готово:** не входит в текущий MVP-срез.
...
## M1.08 — Синхронное provisioning вместо lazy repair

**Зависит от:** M1.03, M1.05.

- [x] Identity вызывает внутреннюю идемпотентную команду Profile до собственного commit;
- [x] пользовательские GET остаются read-only;
- [x] durable reconciliation для Profile `204` с последующим отказом Identity commit.

**Готово:** успешная новая регистрация синхронно подтверждает profile + settings;
скрытый repair в read path отсутствует.

## M1.09 — Закрыть Gate G1

- [x] register → profile → Identity commit/outbox E2E;
- [x] duplicate provisioning/retry test;
- [x] concurrent PATCH test;
- [x] auth negative tests;
- [x] docs/readiness.
